...
class StarRating:
    ...
    def update(self):
        ...
        # Calculer combien d'étoiles devraient être affichées à l'instant
        # présent
        should_show_stars = min(
            self.score, int(
                current_time / self.delay_per_star) + 1)
    ...
    def add_star_particles(self, star_index):
        star_x = self.rect.x + (star_index + 0.5) * \
            (self.rect.width // self.num_stars)
...
class CompareDraw:
    ...
    def generate_comparison_lines(self):
        ...
        for _ in range(num_lines):
            ...
            # Point correspondant sur le dessin du joueur (avec un léger
            # décalage pour l'effet)
            offset = 20
            x2 = max(0, min(self.original_image.get_width(),
                     x1 + random.randint(-offset, offset)))
            y2 = max(0, min(self.original_image.get_height(),
    ...
    def draw(self, surface):
        ...
        scaled_original = pygame.transform.scale(
            self.original_image, (inner_rect.width, inner_rect.height))
        scaled_player = pygame.transform.scale(
            self.player_drawing, (inner_rect.width, inner_rect.height))
        ...
        # Dessiner les lignes de comparaison quand on montre le dessin du
        # joueur
        if not self.show_original:
            for line in self.comparison_lines:
                if line['alpha'] > 0:
                    # Ajuster les coordonnées aux coordonnées du cadre
                    start_x = inner_rect.x + \
                        line['start'][0] * inner_rect.width / self.original_image.get_width()
                    start_y = inner_rect.y + \
                        line['start'][1] * inner_rect.height / self.original_image.get_height()
                    end_x = inner_rect.x + \
                        line['end'][0] * inner_rect.width / self.original_image.get_width()
                    end_y = inner_rect.y + \
                        line['end'][1] * inner_rect.height / self.original_image.get_height()

                    # Créer une couleur avec alpha
                    color_with_alpha = (
                        *line['color'], int(255 * line['alpha']))
...
class Confetti:
    ...
    def start(self):
        ...
        for _ in range(200):
            ...
            self.particles.append(
                {'x': x, 'y': y, 'speed': speed, 'size': size, 'color': color})
