...
class SoloPlay:
    def __init__(self, screen, cursor, model_path, achievements_manager):
        ...
        while running:
            ...
            if self.mouse_down and self.validate_button_rect.collidepoint(
                    self.mouse_pos) and self.can_draw:  # Validation du dessin
               ...
               # end game
                for i in range(len(SOLO_THEMES)):
                    for j in range(len(SOLO_THEMES[i]["images"])):
                        if SOLO_THEMES[i]["images"][j]["path"] == model_path.split(
                                "/")[-1]:
                            SOLO_THEMES[i]["images"][j]["num_try"] += 1
                            if self.similarity_score >= 70:
                                SOLO_THEMES[i]["images"][j]["stars"] = max(
                                    SOLO_THEMES[i]["images"][j]["stars"], 3)
                                if self.similarity_score >= 90:
                                    self.achievements_manager.new_achievement(
                                        4)
                            elif self.similarity_score >= 50:
                                SOLO_THEMES[i]["images"][j]["stars"] = max(
                                    SOLO_THEMES[i]["images"][j]["stars"], 2)
                            elif self.similarity_score >= 30:
                                SOLO_THEMES[i]["images"][j]["stars"] = max(
                                    SOLO_THEMES[i]["images"][j]["stars"], 1)
                            break
                save_data("PLAYER_DATA", "SOLO_THEMES")

                pygame.image.save(self.canvas_surf, "assets/your_best_draws/" +
                                  str(PLAYER_DATA["solo_game_played"]) + ".png")

                self.popup_result.start(
                    self.similarity_score, model_path, self.canvas_surf)
    ...
    def define_layout(self, model_path):
        ...
        self.canvas_rect = pygame.Rect(
            # marge à gauche
            int((self.W - 0.90 * self.H - 40 - palette_width) // 2),
    ...
    def draw_canvas(self):
        ...
        if self.can_draw:
            ...
            if self.mouse_down and self.canvas_rect.collidepoint(
                    self.mouse_pos):
                ...
                if self.last_canvas_pos:  # Vérifie qu'on a une position précédente
                    ...
                    # Dessine une ligne entre la dernière position et la
                    # nouvelle
                    pygame.draw.line(
                        self.canvas_surf,
                        self.pen_color,
                        ...
                        self.pen_radius * 2)
                    pygame.draw.circle(
                        self.canvas_surf, self.pen_color, (local_x, local_y), self.pen_radius)
