...
class AvatarManager:
    ...
    def draw(self):
        ...
        # Afficher le pseudo
        pseudo_surf = MEDIUM_FONT.render(
            self.input_text +
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if avatar_manager.handle_event(
                event, mouse_pos, pygame.mouse.get_pressed()):
    ...
    if current_page == "menu":
        ...
        if not (
                avatar_manager.show_buttons or avatar_manager.is_expanding or avatar_manager.is_retracting):
            ...
            for _ in range(500):
                px = random.randint(
                    main_panel_x, main_panel_x + main_panel_width)
                py = random.randint(
                    main_panel_y, main_panel_y + main_panel_height)
            ...
            for i, button_text in enumerate(buttons):
                ...
                # Animation de survol
                hover = button_x <= mouse_pos[0] <= button_x + \
                    button_width and button_y <= mouse_pos[1] <= button_y + button_height

                # Dessiner l'ombre du bouton
                pygame.draw.rect(
                    screen,
                    DARK_BEIGE if not hover else GRAY,
                    (button_x + buttons_offsets,
                     button_y + buttons_offsets,
                     button_width,
                     button_height),
                    border_radius=40)
                ...
                # Texte du bouton avec un léger déplacement lorsqu'il est
                # survolé
                ...
                hover_credit = math.sqrt((mouse_pos[0] - credit_button_x)**2 + (
                    mouse_pos[1] - credit_button_y)**2) <= credit_button_radius
                # Dessiner le cercle
                pygame.draw.circle(
                    screen,
                    DARK_BEIGE if not hover_credit else GRAY,
                    (credit_button_x + buttons_offsets - 1,
                     credit_button_y + buttons_offsets),
                ...
                # Texte du bouton CREDit avec un léger déplacement lorsqu'il
                # est survolé
