...
frames_per_dimensions = {(200, 200): {"image": pygame.image.load(
    "tests/gallery/assets/frame6.png"), "x": 24, "y": 39, "w": 150, "h": 129}}
...
for i in range(len(rooms)):
    for j in range(len(rooms[i]["draws"])):
        if rooms[i]["draws"][j]["path"] is not None:
            rooms[i]["draws"][j]["image"] = pygame.image.load(
                "tests/gallery/draws/" + rooms[i]["draws"][j]["path"]).convert()

            image_w, image_h = rooms[i]["draws"][j]["image"].get_size()

            rooms[i]["draws"][j]["frame"] = frames_per_dimensions[(
                image_w, image_h)]
            rooms[i]["draws"][j]["frame"]["image"] = pygame.transform.scale(
                rooms[i]["draws"][j]["frame"]["image"], (image_w, image_h))

            rooms[i]["draws"][j]["image"] = pygame.transform.scale(
                rooms[i]["draws"][j]["image"],
                (rooms[i]["draws"][j]["frame"]["w"],
                 rooms[i]["draws"][j]["frame"]["h"]))
...
while True:
    ...
    if mouse_x > W - 300:
        view_pos = min(100 + n * 400 - W + 80,
                       view_pos + (mouse_x - (W - 300)) / 14)
    ...
    for i in range(WALL_SIZE):
        ...
        screen.blit(parquet, (-(view_pos %
                                WALL_SIZE) + i * WALL_SIZE, WALL_SIZE))
    ...
    for room in rooms:
        for draw in room["draws"]:
            if draw["path"] is not None:
                ...
                screen.blit(draw["frame"]["image"],
                            (draw["x"] - view_pos, draw["y"]))
